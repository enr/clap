...
public class DefaultConfiguration implements Configuration {
    ...
    /*
     * A map with keys configuration files paths and value a boolean indicating if the file has been loaded, which configuration has attemped to load
     */
    Map<String, Boolean> paths = Maps.newHashMap();
    
    @Inject
    public DefaultConfiguration(AppMeta meta, EnvironmentHolder environment, ConfigurationReader configurationReader, Reporter reporter) {
    	this.meta = meta;
    ...
    private String installationConfigurationPath() {
        ...
        StringBuilder sb = new StringBuilder().append(configurationDirectory).append(File.separator).append(meta.name()).append(".groovy");
        return sb.toString();
	}
    ...
    /*
     * returns true if configuration file exists and it has been loaded and parsed correctly
     */
     ...
     *
    private String getFileFromConfigurationDirs(String filename) {
    	List<File> configurationDirectories = Lists.newArrayList(environment.installationConfigurationDirectory(),
    				environment.systemConfigurationDirectory(), environment.userConfigurationDirectory());
    	for (File dir : configurationDirectories) {
			File file = new File(dir, filename);
			reporter.info("looking for dataset configuration file:%n- %s", file.getAbsolutePath());
			if (file.exists()) {
